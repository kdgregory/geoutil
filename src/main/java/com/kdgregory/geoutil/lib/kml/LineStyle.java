...
//----------------------------------------------------------------------------
    ...
    public static LineStyle fromXml(Element elem)
    {
        ...
        ObjectUtils.optSetString(XmlUtils.getChildText(elem, namespace, KmlConstants.E_LINESTYLE_COLOR), s::setColor);

        String colorMode = XmlUtils.getChildText(elem, namespace, KmlConstants.E_LINESTYLE_COLORMODE);
        if (colorMode != null)
        {
